...
class View
{
    ...
    public function htmlViewer($html_template, $vars = '', $mode = "echo") : ?string
    {
        ...
        if ($mode === "echo") {
            //если запрошена верхняя часть страницы
            //отправляем заголовки запрета кеширования
            //страницы и устанавливаем кодировку utf-8
    ...
    private function templateParser($html_template, $vars = '')
    {
        ...
        @$replaced_template = str_replace(
            '<!--TITLE-->',
            $this->localisation->page_title[$vars],
            $replaced_template
        );
        //замена ключевых слов в шаблонах на PHP-код
        $this->parsed_html = preg_replace(
            $this->parse_table["regex"][0][0],
            $this->parse_table["regex"][0][1],
            $replaced_template
        );
